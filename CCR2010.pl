...
#printez in log numele fisierului, aka numele dosarului	
	open (MYFILE, '>>log.txt');
	print MYFILE "$nume ";
	close (MYFILE);
...
s/<!DOCTYPE HTML PUBLIC "-\/\/W3C\/\/DTD HTML 4.0 Transitional\/\/EN">\n<HTML>/<?xml version="1.0" encoding="ISO-8859-2"?>\n<lex>/s
	...
	if ($decizie =~ m/<P(.+?)DECIZIA(\s|\n)Nr\.(\d+)</ ) { #am scos in 2010 \/B><\/P> la final; !!am scos \s de dupa P
		$temporar = $3;
		#bag numarul deciziei in fisier
		open (MYFILE, '>>log.txt');
		print MYFILE "$temporar ";
		close (MYFILE);
		$decizie =~ s/<lex>\n/<lex>\n<emitent id="CCR" \/>\n<tip id="decizie" \/>\n<nr id="$temporar" \/>\n/s;
		}
	else {
		if ($decizie =~ m/<P(.+?)HOT.R.REA(\s|\n)Nr\.(\d+)</ ) { #am scos in 2010 \/B><\/P> la final; !!am scos \s de dupa P
			$temporar = $3;
			$temporar1 = 'H'.$3; #daca avem hotarari, nu decizii, le marchez cu un H in fata numarului; pentru asta creez o variabila noua, ca sa nu afecteze codul pentru identificarea datei
			#bag numarul deciziei in fisier
			open (MYFILE, '>>log.txt');
			print MYFILE "$temporar1 ";
			close (MYFILE);
			$decizie =~ s/<lex>\n/<lex>\n<emitent id="CCR" \/>\n<tip id="hotarare" \/>\n<nr id="$temporar1" \/>\n/s;
			}
		else {
			if ($decizie =~ m/<P(.+?)DECIZIA(\s|\n)Nr\.(\d)\.(\d+)</ ) { #decizie peste 1000
				$temporar1 = $3;
				$temporar2 = $4;
				$temporar = $3.$4;
				#bag numarul deciziei in fisier
				...
				close (MYFILE);
				$decizie =~ s/<lex>\n/<lex>\n<emitent id="CCR" \/>\n<tip id="decizie" \/>\n<nr id="$temporar" \/>\n/s;
				$temporar = $temporar1.'.'.$temporar2;
				}
			else
				{print "ATENTIE: Nu am gasit numarul deciziei in $cod \n";
				#marchez in log si daca nu gasesc numarul deciziei
				open (MYFILE, '>>log.txt');
				print MYFILE "WTF ";
				close (MYFILE);
				}
	...
	#Aici a fost o problema la dosarul 2008/1058, a luat data aiurea, de corectat
	if ($decizie =~ m/>DECIZIA(\s|\n)Nr\.$temporar<(.+?)>\n(.+)>din\s(\d+)\s(\w+)\s(\d+)</s) { #am inlocuit in 2010 \/B><\/P cu (.+?)
		my $zi = $4;
		my $lu = $5;
		my $an = $6;
		#marchez in log data deciziei
		open (MYFILE, '>>log.txt');
		print MYFILE "$an $lu $zi\n";
		close (MYFILE);
		$decizie =~ s/<nr\sid="$temporar" \/>\n/<nr id="$temporar" \/>\n<date>\n<zi id="$zi" \/>\n<luna id="$lu" \/>\n<an id="$an" \/>\n<\/date>\n/s;
	}
	else { #aici incercam sa vedem daca e hotarare mai intai, doar dupa aia marcam eroarea
		if ($decizie =~ m/>HOT.R.REA(\s|\n)Nr\.$temporar<(.+?)>\n(.+)>din\s(\d+)(\s|\n)(\w+)(\s|\n)(\d+)</s) { #am inlocuit in 2010 \/B><\/P cu (.+?)
		my $zi = $4;
		my $lu = $6;
		my $an = $8;
		#marchez in log data deciziei
		...
		$decizie =~ s/<nr\sid="$temporar" \/>\n/<nr id="$temporar" \/>\n<date>\n<zi id="$zi" \/>\n<luna id="$lu" \/>\n<an id="$an" \/>\n<\/date>\n/s;
			...
			print "ATENTIE: Nu am gasit data deciziei in $cod \n";
			#marchez in fisier si daca nu a gasit data deciziei
			open (MYFILE, '>>log.txt');
			print MYFILE "WTF WTF WTF\n";
			close (MYFILE);
			}
		}
	#BUCATA CARE URMEAZA NU E EXECUTATA
	#############################################
	#############################################
